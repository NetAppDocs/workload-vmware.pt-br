...
Dependendo da política e das permissões que você adicionou à sua conta de fábrica de carga de trabalho, você pode implantar o sistema de arquivos FSX for ONTAP completamente usando a fábrica de carga de trabalho (usando o modo leitura/gravação). Se você tiver menos permissões (modo somente leitura), ou nenhuma permissão (modo básico), você precisará usar as informações do CloudFormation do Codebox e implantar o sistema de arquivos FSX for ONTAP na AWS.
