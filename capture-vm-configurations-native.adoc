...
+
No centro de planejamento, você pode carregar seus dados de inventário de VM, ver uploads de dados de inventário anteriores e planejar uma migração usando qualquer um dos conjuntos de dados de inventário de VM. e então selecionar *Migração para computação nativa da AWS*.  A tela inicial do consultor de migração do VMware é exibida.
